...
int findMorePriorindex(struct process pr[], set<int> arrived){
    int pMin = *(arrived.begin());
    // cout << endl;
    for (int i : arrived)
    {
        // cout << pr[i].id << " ";
        if (pr[pMin].prior > pr[i].prior)
            pMin = i;
    }
    // cout << endl;
    return pMin;
...
int main()
{
    ...
    cout<<"<-------------Non-Preemptive PRIORITY ALGO------------->\n\n";
    printf("Process\tBT\tAT\tPriority\n");
    for (int i = 0; i < 10; i++)
    {
        printf("P%d\t%d \t%d\t%d\n", pr[i].id, pr[i].bt, pr[i].at, pr[i].prior);
        // printf("hes\n");
    }

    // sorting the processes with respect to at & priority
    struct process temp;
    for (int i = 1; i < 10; i++)
    {
        for (int j = 0; j < i; j++)
        {
            ...
            if(pr2[i].at == pr2[j].at){
                if(pr2[i].prior < pr2[j].prior){
                    temp = pr2[i];
                    pr2[i] = pr2[j];
                    pr2[j] = temp;
                }
                else if(pr2[i].prior == pr2[j].prior){
    ...
    while (arrived.empty() == false || remaining.empty() == false)
    {
        ...
        {
            j = findMorePriorindex(pr2, arrived);
